...
local function delete_buffer(buffer_number)
	buffer_number = buffer_number or vim.api.nvim_get_current_buf()
	...
	if vim.bo[buffer_number].modified then
		...
		vim.notify("Save changes before closing " .. name, vim.log.levels.WARN)

		return
	end

	local success, error_message = pcall(vim.api.nvim_buf_delete, buffer_number, {
		force = false,
	})

	if not success then
		vim.notify(tostring(error_message), vim.log.levels.ERROR)
	end
end
...
for buffer_index = 1, 9 do
	map("n", "<leader>b" .. buffer_index, string.format("<cmd>BufferLineGoToBuffer %d<CR>", buffer_index), {
		desc = string.format("Go to buffer %d", buffer_index),
	})
end
